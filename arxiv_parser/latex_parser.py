...
def resolve_input_files(tex_content: str, base_dir: str, visited: set = None) -> str:
    """Recursively resolve and inline all \\input{} commands in LaTeX content."""
    if visited is None:
        visited = set()
        ...
        if os.path.exists(full_path):
            ...
            try:
                with open(full_path, 'r', encoding='utf-8', errors='ignore') as f:
                    ...
                    included_content = resolve_input_files(included_content, os.path.dirname(full_path), visited)
    ...
    resolved_content = re.sub(input_pattern, replace_input, tex_content)
    return resolved_content


def extract_title(tex_content: str) -> Optional[str]:
...
def extract_abstract(tex_content: str) -> Optional[str]:
